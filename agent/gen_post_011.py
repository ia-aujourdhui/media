...
def render_slide(w, h, page, total, kicker, title_lines, body_lines=None, cta=False):
    ...
    if cta:
        ...
        draw.text((margin, h - int(h * 0.14)), "un outil IA gratuit chaque jour", font=f_body, fill=GREY)
...
SLIDES = [
    dict(
        kicker="Outil du jour",
        title=[u"Google vient de sortir sa réponse à Suno"],
        body=[(None, u"Et elle est gratuite, sans carte bancaire.")],
    ),
    dict(
        kicker="C'est quoi",
        title=[u"Google Flow Music"],
        body=[(None, u"L'ex-ProducerAI, le studio musical IA de Google."),
              (None, u"Propulsé par Gemini et le modèle musical Lyria 3."),
              (None, u"Accessible sur flowmusic.google, en français.")],
    ),
    dict(
        kicker=u"Comment ça marche",
        title=[u"Tu décris, il compose"],
        body=[("*", u"Tu tapes un style ou une ambiance en langage naturel"),
              ("*", u"L'IA génère un morceau complet, paroles + instru"),
              ("*", u"Jusqu'à 3 minutes de musique, en une seule fois")],
    ),
    dict(
        kicker="Ce qui change vs Suno",
        title=[u"Tu retouches sans tout refaire"],
        body=[("*", u"Tu sélectionnes juste une section pour la réécrire"),
              ("*", u"Cover : change le style en gardant la mélodie"),
              ("*", u"Génère aussi un clip vidéo avec Veo 3.1")],
    ),
    dict(
        kicker="Exports",
        title=[u"Tu repars avec tout le nécessaire"],
        body=[("*", u"Formats mp3, wav, m4a et mp4"),
              ("*", u"Pistes séparées (stems) pour remixer toi-même"),
              ("*", u"Partage par lien ou playlist publique")],
    ),
    dict(
        kicker="Prix",
        title=[u"Gratuit pour commencer"],
        body=[(None, u"L'usage de base est gratuit, sans CB."),
              (None, u"Formules payantes dès 8$/mois si tu veux"),
              (None, u"plus de générations et plus de morceaux en simultané.")],
    ),
    dict(
        kicker=None,
        title=[u"On teste un outil IA gratuit chaque jour"],
        body=None,
        cta=True,
    ),
]
